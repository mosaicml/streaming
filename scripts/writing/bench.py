...
from streaming.base.format.base.tabulation import Tabulator
...
def _parse_args() -> Namespace:
    ...
    args.add_argument('--formats', type=str, default='csv,jsonl,mds')
...
def _generate(split2size: Dict[str, int],
              seed: int = 0x1337,
              pos_prob: float = 0.75,
              low: int = -1_000_000_000,
              high: int = 1_000_000_000,
...
def _write_csv(nums: List[int],
    ...
    out = CSVWriter(out=root, columns=columns, size_limit=size_limit)
    with out:
...
def _write_jsonl(nums: List[int],
    ...
    out = JSONWriter(out=root, columns=columns, size_limit=size_limit)
    with out:
        each_sample = zip(nums, txts)
        if show_progress:
            each_sample = tqdm(each_sample, total=len(nums), leave=False)
...
def _write_mds(nums: List[int],
    ...
    out = MDSWriter(out=root, columns=columns, size_limit=size_limit)
    with out:
        each_sample = zip(nums, txts)
        if show_progress:
            each_sample = tqdm(each_sample, total=len(nums), leave=False)
...
def main(args: Namespace) -> None:
    ...
    t0 = time()
    dataset = _generate(split2size, args.seed, args.data_pos_prob, args.data_low, args.data_high,
                        show_progress)
    elapsed = time() - t0
    ...
    for split in _splits_by_size(dataset):
        ...
        for format_name in format_names:
            ...
            t0 = time()
            try:
                obj = write(nums, txts, split_root)
                if obj:
                    names_objs.append((format_name, obj))
            except:
                raise  # Getting Delta Java OOMs at gigabyte size.
            elapsed = time() - t0
