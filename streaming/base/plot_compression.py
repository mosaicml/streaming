...
def parse_args() -> Namespace:
    ...
    args.add_argument('--encode_rates', type=str, default='encode_rates.png')
    args.add_argument('--encoded_sizes', type=str, default='encoded_sizes.png')
    args.add_argument('--decode_rates', type=str, default='decode_rates.png')
...
def plot_encode_rates(data: List[Datum], algo2color: Dict[str, Optional[str]], dpi: int,
    ...
    plt.title('encoding rate by size')
    plt.xlabel('size (bytes)')
    plt.ylabel('encoding rate (bytes / sec)')
    plt.legend()
...
def plot_decode_rates(data: List[Datum], algo2color: Dict[str, Optional[str]], dpi: int,
    ...
    plt.title('decoding rate by size')
    plt.xlabel('size (bytes)')
    plt.ylabel('decoding rate (bytes / sec)')
    plt.legend()
...
def main(args: Namespace) -> None:
    ...
    plot_encode_rates(data, algo2color, args.dpi, args.font_size, args.line_width,
                      args.encode_rates)
    plot_encoded_sizes(data, algo2color, args.dpi, args.font_size, args.line_width,
                       args.encoded_sizes)
    plot_decode_rates(data, algo2color, args.dpi, args.font_size, args.line_width,
                      args.decode_rates)
