...
from streaming.base.format.dbs.type.base import SimpleVarLeaf


class NDArray(SimpleVarLeaf):
    """np.ndarray DBS type.
    ...
    Args:
        ...
        shape (TyTuple[int], optional): The shape, if fixed. Defaults to ``None``.
    """
    ...
    @classmethod
    def _get_static_size(cls, dtype: TyOptional[str],
                         shape: TyOptional[TyTuple[int]]) -> TyOptional[int]:
        """Get the fixed size of the column in bytes, if applicable.

        Args:
            dtype (str, optional): The dtype, if fixed.
            shape (TyTuple[int], optional): The shape, if fixed.
    ...
    def __init__(self, dtype: TyOptional[str] = None, shape: TyOptional[TyTuple[int]] = None):
    ...
    @classmethod
    def _rightsize_shape_dtype(cls, shape: TyNDArray[np.int64]) -> str:
        """Get the smallest unsigned int dtype that will accept the given shape.

        Args:
            shape (TyNDArray[np.int64]): The shape.
    ...
    def encode(self, obj: TyNDArray) -> bytes:
        """Serialize the given data from the original object to bytes.

        Args:
            obj (TyNDArray): Deserialized object.
    ...
    def decode(self, data: bytes) -> TyNDArray:
        """Deserialize the given data from bytes to the original object.

        Args:
            data (bytes): Serialized data.

        Returns:
            TyNDArray: Deserialized object.
        """
        index = 0
