...
class JPEGArray(Encoding):
    ...
    def encode(self, obj: Sequence[bytearray]) -> bytes:
        ...
        image_byte_sizes_array: npt.NDArray = np.uint32(image_byte_sizes)
        image_byte_sizes_as_bytes: bytes = image_byte_sizes_array.tobytes()
        bytes_iterables: Iterator[bytes] = chain([n_images_as_bytes], [image_byte_sizes_as_bytes], obj)
    ...
    def decode(self, data: bytes) -> List[Image.Image]:
        ...
        n_bytes_per_image: Sequence[int] = np.frombuffer(data[4:image_bytes_offset], dtype=np.uint32).tolist()
