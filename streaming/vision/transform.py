...
class RandomResizePair(torch.nn.Module):
    ...
    def forward(self, sample: Tuple[Image.Image, Image.Image]):
        image, target = sample
        resize_scale = np.random.random_sample() * (self.max_scale -
        ...
        resized_target = TF.resize(target, resized_dims,
...
class PadToSize(torch.nn.Module):
    ...
    def forward(self, image: Image.Image):
        ...
        image = TF.pad(image, padding,
                       fill=self.fill)  # type: ignore - transform typing does not include PIL.Image
        return image
    ...
    def forward(self, image: Image.Image):
        if np.random.randint(2):
            ...
            image = TF.adjust_brightness(
                image,
                brightness_factor)  # type: ignore - transform typing does not include PIL.Image
        ...
        if np.random.randint(2):
            ...
            image = TF.adjust_saturation(
                image,
                saturation_factor)  # type: ignore - transform typing does not include PIL.Image

        if np.random.randint(2):
            hue_factor = np.random.uniform(-self.hue, self.hue)
            image = TF.adjust_hue(
                image, hue_factor)  # type: ignore - transform typing does not include PIL.Image
