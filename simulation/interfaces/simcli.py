...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Simulate your training yaml from the command line.')
    ...
    parser.add_argument('-d', '--devices', type=int, help='number of devices per node', required=False)
    parser.add_argument('-t', '--time_per_sample', type=float, help='time to process one sample on one device (seconds)', required=False)
    parser.add_argument('-b', '--node_internet_bandwidth', type=str, help='internet bandwidth per node (bytes/s)', required=False)
    args = parser.parse_args()

    # Read in yaml file
    filepath = args.file
    total_devices, workers, max_duration, global_batch_size, train_dataset = ingest_yaml(filepath=filepath)
    ...
    if node_network_bandwidth is None:
        ...
        try:
            ...
            node_network_bandwidth = bandwidth_input
    ...
    # Simulate Run
    step_times, step_downloads, startup_time, min_cache_limit = next(
        simulate(dataset, time_per_sample, node_network_bandwidth, max_duration=max_duration))
    ...
    print("\nSimulation Stats:")
    print(f"Minimum cache limit needed: {min_cache_limit:,} bytes")
    ...
    if warmup_step == total_batches:
        ...
        print('⚠️ This configuration experiences some downloading-related slowdowns even after \
              warmup.')
    print("{0} steps, or {1:.1f}% of all steps, waited for shard \
          downloads.".format(all_throughput_drops, 100*all_throughput_drops/(total_batches)))
    if warmup_step != total_batches:
        ...
        print("There were {} steps that waited for shard downloads after the warmup \
